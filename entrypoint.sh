...
MAXRATE=$(($BITRATE + 128))
if [ -z ${BUFSIZE+x} ]; then BUFSIZE=$((($MAXRATE + 3 + 1) / 3)); fi
...
if [ -z ${SCALER+x} ]; then SCALER=area; fi
...
if [ -z ${FFMPEG_ARGS+x} ]; then FFMPEG_ARGS="-s ${RESOLUTION} -c:v libx264 -preset ${PRESET} -profile:v ${PROFILE} -r ${FRAMERATE} -g ${FRAMERATE_2X} -x264-params \"bitrate=${BITRATE}:vbv_maxrate=${MAXRATE}:vbv_bufsize=${BUFSIZE}:threads=${THREADS}:bframes=${BFRAMES}:rc_lookahead=${RC_LOOKAHEAD}:keyint=${FRAMERATE_2X}:min-keyint=${FRAMERATE_2X}:nal_hrd=cbr:scenecut=0:rc=cbr:force-cfr=1\" -sws_flags ${SCALER} -pix_fmt yuv420p -c:a copy -f flv -strict normal"; fi
