...
if [ -z ${FFMPEG_ARGS+x} ]; then FFMPEG_ARGS="-s ${RESOLUTION} -r ${FRAMERATE} -vf \"fps=${FRAMERATE}\" -c:v libx264 -preset ${PRESET} -profile:v ${PROFILE} -g ${FRAMERATE_2X} -x264-params \"bitrate=${BITRATE}:vbv_maxrate=${BITRATE}:vbv_bufsize=${BUFSIZE}:threads=${THREADS}:bframes=${BFRAMES}:rc_lookahead=${RC_LOOKAHEAD}:keyint=${FRAMERATE_2X}:min-keyint=${FRAMERATE_2X}:nal_hrd=cbr:scenecut=0:rc=cbr:force-cfr=1\" -sws_flags ${SCALER} -pix_fmt yuv420p -c:a copy -f flv -strict normal"; fi
...
rtmp {
  server {
    ...
    application livein {
      live on;
      record off;
      exec ffmpeg -r ${FRAMERATE} -i "rtmp://127.0.0.1/livein/${STREAM_KEY}" ${FFMPEG_ARGS} "rtmp://127.0.0.1/liveout";
    }
